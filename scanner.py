"""
NSE Camarilla Volume Scanner — Robust v3.0
==========================================
Improvements over v2.0:
  • Python 3.8+ compatible type hints (no dict|None union syntax)
  • Concurrent scanning via ThreadPoolExecutor (6 workers default)
  • Single shared requests.Session with automatic retry + backoff
  • Bearish signal support  (--bearish flag / bearish=True)
  • Configurable 52-week window via HV_DAYS env var
  • cfg_override pattern so dashboard can pass per-scan settings
  • ETA counter in progress callback
  • Better score normalisation (bearish-aware)
...
def run_scan(
    ...
    def _task(ticker: str) -> Optional[Dict]:
        ...
        log.info(f"[{n}/{total}] {mode} {ticker} → {'✅ SIGNAL' if result else 'skip'}")
...
def print_table(signals: List[Dict]) -> None:
    ...
    print("=" * 105)
    hdr = (f"  {'SYMBOL':<14}{'SCORE':>6} {'PRICE':>10} {'ENTRY':>10} "
           f"{'TARGET':>10} {'STOPLOSS':>10} {'UPSIDE':>7} {'R:R':>5} {'VOL':>5} CANDLE")
    print(hdr)
    print("-" * 105)
    for r in signals:
        ...
        print(
            ...
            f"{r['upside']:>+6.1f}% {rr:>4}  {r['vol_ratio']:>4.1f}x {r['candle']}"
        )
    print("=" * 105)
...
def send_telegram(
    ...
    for r in signals[:15]:
        ...
        lines.append(
            f"{emoji} *{r['symbol']}* Score: {r['score']}/100\n"
            f"  Price: ₹{r['price']:,.2f}\n"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="NSE Camarilla Volume Scanner v3.0")
